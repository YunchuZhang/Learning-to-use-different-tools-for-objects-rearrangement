...
def main(unused_argv):
	...
	# Load test dataset.
	ds = dataset.Dataset(os.path.join(FLAGS.data_dir, f'{FLAGS.task}-train'))

	# Run testing for each training run.
	for train_run in range(FLAGS.n_runs):
		name = f'{FLAGS.task}-{FLAGS.agent}-{FLAGS.n_demos}-{train_run}-sweep'
		...
		for i in range(ds.n_episodes):
			...
			img, p0, p0_theta, p1, p1_theta = agent.get_sample(ds)
			# p0 (u,v) in pixel y, x 
			# image = cv2.circle(img, center_coordinates, 1, (0,0,255), 1)
